import { NumericMaskConfig, EqpNumericInputMode } from "./eqp-numeric.config";
...
export class InputService {
  ...
  constructor(private htmlInputElement: any, private options: NumericMaskConfig) {
  ...
  addNumber(keyCode: number): void {
    ...
    if (!this.rawValue) {
      ...
      if (inputMode === EqpNumericInputMode.NATURAL && precision > 0) {
      ...
      if (inputMode === EqpNumericInputMode.NATURAL && inDecimalPortion && selectionStart === selectionEnd) {
      ...
      // Se il valore precedente alla posizione del cursore è un decimale e viene scritto il simbolo decimale/migliaia allora mi sposto alla selezione successiva
      let rawValueEndIsValid: boolean = (rawValueEnd[0] == this.options.decimal) || (rawValueEnd[0] == this.options.thousands) && (keyChar == this.options.decimal || keyChar == this.options.thousands);
      if (isDecimalOrThousands && (rawValueEndIsValid && selectionStart == this.rawValue.indexOf(decimal))) {
        // If the cursor is just before the decimal or thousands separator and the user types the
        // decimal or thousands separator, move the cursor past it.
        nextSelectionStart++;
        //Forzo lo spostamento della selezione dopo i decimali
      } else if (keyChar == this.options.thousands || keyChar == this.options.decimal && (decimal != '' && decimal != null && precision != 0)) {
        let decimalPosition = this.rawValue.indexOf(decimal);
        if (decimalPosition != -1 && selectionStart < decimalPosition)
          nextSelectionStart = decimalPosition + 2;
      } else if (!this.SINGLE_DIGIT_REGEX.test(keyChar)) {
        // Ignore other non-numbers.
        return;
      }

      //Se la stringa finisce con un separatore, allora non aggiorno i valori ma semplicemente sposto il selettore
  ...
  applyMask(isNumber: boolean, rawValue: string, disablePadAndTrim = false): string {
    ...
    if (inputMode === EqpNumericInputMode.NATURAL && !isNumber && !disablePadAndTrim) {
  ...
  removeNumber(keyCode: number): void {
    ...
    if (selectionEnd === selectionStart) {
      if (keyCode == 8) {
        ...
        if (inputMode === EqpNumericInputMode.NATURAL && isCursorInDecimals) {
          shiftSelection = -1;
    ...
    if (inputMode === EqpNumericInputMode.NATURAL && selectionStart > decimalIndex) {
